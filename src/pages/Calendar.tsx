...
import { Calendar, CalendarDays, Clock, Facebook, Instagram, Linkedin, Plus, Filter } from "lucide-react";
...
export default function CalendarView() {
  ...
  const PostCard = ({ post }: { post: ScheduledPost }) => (
    <div
      ...
      style={{ backgroundColor: `hsl(var(--${statusColors[post.status as keyof typeof statusColors].replace('bg-', '')}))` }}
      onClick={() => setSelectedPost(post)}
    >
      <div className="flex items-center space-x-1 mb-1">
        {post.target_platforms.map((platform) => {
          const Icon = platformIcons[platform as keyof typeof platformIcons];
          return Icon ? <Icon key={platform} className="h-3 w-3 text-white" /> : null;
        })}
      </div>
      ...
      <p className="text-white/80">
        {post.scheduled_for ? format(parseISO(post.scheduled_for), 'HH:mm') : ''}
      </p>
    </div>
  );
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Badge 
                    className={`${statusColors[selectedPost.status as keyof typeof statusColors]} text-white`}
                  >
                    {selectedPost.status}
                  </Badge>
                </div>
                  ...
                  <div className="flex items-center space-x-2 text-muted-foreground">
                    ...
                    <span>
                      {selectedPost.scheduled_for 
                        ? format(parseISO(selectedPost.scheduled_for), 'MMM d, yyyy at h:mm a')
                        : 'Not scheduled'
                      }
                    </span>
                  </div>
