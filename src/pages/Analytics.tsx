...
const statusConfig = {
  completed: { color: 'text-success', bg: 'bg-success', icon: CheckCircle, label: 'Published' },
  failed: { color: 'text-destructive', bg: 'bg-destructive', icon: XCircle, label: 'Failed' },
  pending: { color: 'text-warning', bg: 'bg-warning', icon: Clock, label: 'Pending' },
  processing: { color: 'text-primary', bg: 'bg-primary', icon: RefreshCw, label: 'Processing' }
};
...
export default function Analytics() {
  ...
  const fetchAnalyticsData = async () => {
    ...
    try {
      ...
      const publishedPosts = jobsData?.filter(job => job.status === 'completed').length || 0;
      const failedPosts = jobsData?.filter(job => job.status === 'failed').length || 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-success">{analyticsData.publishedPosts}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-destructive">{analyticsData.failedPosts}</div>
            <p className="text-xs text-muted-foreground">
            ...
            <Table>
              ...
              <TableBody>
                {jobLogs.map((log) => {
                  ...
                  const PlatformIcon = platformIcons[log.platform as keyof typeof platformIcons];
                  ...
                  return (
                    <TableRow key={log.id}>
                      ...
                      <TableCell>
                        <span className={log.attempts > 1 ? 'text-warning' : ''}>
                          {log.attempts}/3
                        </span>
                      </TableCell>
                          ...
                          {log.platform_post_id && (
                            <Button
                              ...
                              onClick={() => {
                                ...
                                window.open(urls[log.platform as keyof typeof urls], '_blank');
                              }}
                            >
