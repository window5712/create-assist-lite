    ...
    for (const job of pendingJobs || []) {
      try {
        ...
        const accessToken = await decryptToken(
          job.social_account.access_token_encrypted
        );
...
function getPlatformContent(post: any, platform: string) {
  ...
  return {
    ...
    call_to_action:
      platformVariants[platform]?.call_to_action || post.call_to_action,
  };
}
...
async function publishToFacebook(
  ...
  if (content.media_urls && content.media_urls.length > 0) {
    if (content.media_urls.length === 1) {
      ...
      postData.attached_media = content.media_urls.map(
...
async function publishToInstagram(
  ...
  if (content.media_urls && content.media_urls.length > 0) {
    ...
    if (mediaData.error) {
      throw new Error(
        `Instagram media creation failed: ${mediaData.error.message}`
      );
    }
...
async function publishToLinkedIn(
  ...
  if (content.media_urls && content.media_urls.length > 0) {
    postData.specificContent["com.linkedin.ugc.ShareContent"].media =
      content.media_urls.map((url: string) => ({
        status: "READY",
        description: {
          text: "Image",
        },
  ...
  if (!response.ok) {
    throw new Error(
      `LinkedIn API error: ${responseData.message || "Unknown error"}`
    );
  }
